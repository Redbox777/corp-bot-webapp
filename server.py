...
def get_player(chat_id):
    ...
    if not row:
        ...
        conn.commit()
        row = c.execute('SELECT * FROM players WHERE chat_id = ?', (chat_id,)).fetchone()
    ...
    for q in QUESTS:
        if q["id"] not in q_data:
            q_data[q["id"]] = {"progress": 0, "claimed": False, "last_date": today}
        if q["daily"] and q_data[q["id"]]["last_date"] != today:
            q_data[q["id"]] = {"progress": 0, "claimed": False, "last_date": today}
...
def click(chat_id):
    ...
    if not row:
        ...
        c.execute('''INSERT INTO players (chat_id, balance, clicks, level, passive_income, click_power, upgrades, 
            ...
            VALUES (?, 10, 1, 1, 0, 10, '{}', ?, '[]', 10, ?, '{}', 0, 1.0)''', (chat_id, time.time(), ref_code))
        conn.commit()
        ...
        conn.commit()
        res = {"balance": nb, "clicks": nc, "level": (nc//100)+1, "click_power": pwr,
               "upgrades": json.loads(row["upgrades"] or "{}"), "total_earned": nt, "prestige_mult": mult}
    conn.close()
    return jsonify(res)
...
def rebirth(chat_id):
    ...
    if lvl < 5 and te < 5000:
        conn.close(); return jsonify({"error": f"Нужен 5 ур. или 5000$ (сейчас: {lvl} / {te})"}), 400
    
    pp = row["prestige_points"] or 0
    gems = max(1, (te // 5000) - pp)
    new_gems = pp + gems
    new_mult = 1.0 + (new_gems * 0.05)
    new_prest = (row["total_prestiges"] or 0) + 1
